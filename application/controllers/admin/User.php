...
class User extends CI_Controller 
{
	...
	public function delete_mail_list(){
	...
	{
		$data['RESULT'] = $this->user_model->get_all_join_wait_list(); 
		$this->load->view('admin/user/join-wait-list',$data);
	}
	
	
	
	public function delete_join_list(){
	    
	    $args = func_get_args();
		$this->db->where('id',$args[0]);
		$this->db->delete('tbl_wait_list');
		$this->session->set_flashdata('msg','<div class="alert alert-success">Record has been successfully deleted.</div>');
		redirect('admin/user/join_wait_list');
	}
	...
	public function download()
	{
	    ...
	    $data['RESULT'] = $this->user_model->get_user_by_id($args[0]); 
		$this->load->view('admin/user/download',$data);
	}
